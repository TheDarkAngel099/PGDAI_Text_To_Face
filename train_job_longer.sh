...
export OUT_DIR="$BASE_DIR/sd_training/outputs_320_1epoch"
export CKPT_DIR="$BASE_DIR/sd_training/checkpoints_320_1epoch"

# Training hyperparameters for RealViz
export RESOLUTION=320                  # Match dataset native resolution (320x320)
export BATCH_PER_GPU=8                 # Increased for better gradient stability
export GRAD_ACCUM=4                    # Effective batch = gpus * BATCH_PER_GPU * GRAD_ACCUM = 32
export LEARNING_RATE=5e-5              # Reduced for stability with larger batch
export LR_SCHEDULER="linear"           # Use linear decay instead of constant
export LR_WARMUP_STEPS=500             # Increased warmup for better stability
export MAX_TRAIN_STEPS=9050            # Five complete epochs: 7238 images / (batch_size=8 × grad_accum=4) × 5 = 9050 steps
export VALIDATION_STEPS=225            # Validate every ~1/8 epoch to monitor quality
export CHECKPOINT_STEPS=905            # Save 10 checkpoints at equal intervals (9050/10 = 905)
export CHECKPOINTS_TOTAL_LIMIT=10      # Keep all 10 checkpoints for comparison
export SEED=42
...
accelerate launch --mixed_precision=fp16 --num_processes=1 --num_machines=1 \
